...
**Helper scripts:**
- `~/.config/quickshell/scripts/bt-devices.sh` → JSON array of paired BT devices with connection state
- `~/.config/quickshell/scripts/wifi-networks.sh` → JSON of saved WiFi networks with active state
- `~/.config/quickshell/scripts/clipboard-entries.sh` → JSON of cliphist entries; decodes binary PNG images to `/tmp/qs-clipboard-cache/`
- `~/.config/quickshell/scripts/app-list.sh` → JSON of all non-hidden .desktop apps with exec, icon paths, terminal flag
...
~/.config/quickshell/DashboardApp/ — Quickshell dashboard (replaces eww)
~/.config/quickshell/BluetoothApp/ — Quickshell BT panel
~/.config/quickshell/WiFiApp/      — Quickshell WiFi panel
~/.config/quickshell/scripts/      — bt-devices.sh, wifi-networks.sh (JSON helpers)
